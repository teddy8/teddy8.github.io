...
task('work1', server)  // 'work1'라는 이름의 promise 생성
  .then(v => {
    console.log(v.result);
    server.traffic -= v.num;
    return task('work2', server); // traffic 70
  })
  .then(v => {
    console.log(v.result);
    server.traffic -= v.num;
    return task('work1', server); // traffic 40 
  })
  .then(v => {
    console.log(v.result);
    server.traffic -= v.num;
    return task('work2', server); // traffic 10 (30보다 낮아서 거부)
  })
